function [Xk] = Algorithm1_TestShrinkDecay(X0, M, P, lambda1,lambda2, mu,kmax,Tol)
...
%while true
    ...
    ind = find(Sw>0);
    subplot(1,3,3)
    semilogy(1:length(Sw(ind)),Sw(ind),'*')

    disp('Size of Shrunk Singular Values:')
    disp(length(ind))
    disp('~~~~~~~~~~~~~~~~')
    disp('')
