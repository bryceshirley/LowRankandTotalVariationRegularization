function subGradTVnorm3D = SubGradTVNorm3D_V2(X)
% Version 2 computes |X(i, j, k) - X(i, j+2, k)|
...
% the sign function is suitable for this task.
    ...
    XF1 = reshape(X,n*m,r);
    XF2 = [XF1(:,(dist+1):end),ones(n*m,dist)];
    TxF = -reshape((XF1-XF2),n,m,r);
    ...
    XR1 = reshape(X,n,m*r);
    XR2 = [XR1((dist+1):end,:);ones(dist,m*r)];
    TxR = -reshape((XR1-XR2),n,m,r);
    ...
    XD1 = reshape(reshape(X,n,m*r)',m,n*r);
    XD2 = [XD1((dist+1):end,:);zeros(dist,n*r)];
    TxD = -reshape(reshape((XD1-XD2),m*r,n)',n,m,r);
    ...
    XB1 = reshape(X,n*m,r);
    XB2 = [ones(n*m,dist),XB1(:,1:end-dist)];
    TxB = reshape((XB1-XB2),n,m,r); 
    ...
    XL1 = reshape(X,n,m*r);
    XL2 = [ones(dist,m*r); XL1(1:end-dist,:)];
    TxL = reshape((XL1-XL2),n,m,r);
    ...
    XU1 = reshape(reshape(X,n,m*r)',m,n*r);
    XU2 = [zeros(dist,n*r);XU1(1:end-dist,:)];
    TxU = reshape(reshape((XU1-XU2),m*r,n)',n,m,r);
    
    %% Compute The Subgradient for Each Term and Sum For Subgradient of TV
    subGradTVnorm3D = (sign(TxL) + sign(TxR) + sign(TxD) + sign(TxU) + ...
        + sign(TxF) +sign(TxB))/(dist*2);
