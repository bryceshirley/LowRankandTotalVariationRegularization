function subGradTVnorm = SubGradTVNorm_Tensor(X,N)
% Computes the sub gradient matrix the total variation norm.
% INPUTS:
% - X: is a Tensor
% - N: is a 1,2 or 3 for mode-n matrix unfoldings
% OUTPUTS:
...
% the sign function is suitable for this task.
    ...
    XRowBoundary = [XN;zeros(1,m)]; % Choose sharp Boundary Values as zeros
    rowNeighboursDiff = - diff(XRowBoundary); % Negate signs

    % b) Compute X(i, j) - X(i, j+1)
    XColBoundary = [XN,zeros(n,1)];
